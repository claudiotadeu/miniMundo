...
operacao = 1
while operacao != 0:
    operacao=menuAgencias()
    match operacao:
        case 1:
            print('NOVA AGENCIA')
            nome=input("Digite o Nome da Agência: ")
            cidade=input("Digite a Cidade da Agência: ")
            ...
            while ativo != 0 and ativo !=1:
                ...
                ativo=validaSituacao(ativo)
            AgenciaController.InserirAgencia(nome,cidade,int(ativo))

        case 2:
            print('ALTERAR AGÊNCIA')
            registro=input('Digite a Cidade da Agência: ')
            resultado=AgenciaController.PesquisaAgencia('cidade',registro)
            if resultado:
                registro=resultado
            else:
                registro=input('Digite o ID do Registro a ser Alterado: ')
            ...
            if confirmacao=="S":
                ...
                AgenciaController.AtualizarAgencia(chave,novoDado,registro)
            else:
                print("Operação Cancelada!")

        case 3:
            print('EXCLUIR AGÊNCIA')
            registro=input('Digite a Cidade da Agência: ')
            resultado=AgenciaController.PesquisaAgencia('cidade',registro)
            if resultado:
                registro=resultado
            ...
            if confirmacao=="S":
                AgenciaController.ExluiAgencia(registro)
            else:
                print("Operação Cancelada!")

        case 4:
            print('LISTAR AGÊNCIAS')
            lista=AgenciaController.listar()
            ...
            for l in lista:
                ...
                if posicao == 1:
                    ...
                    posicao='Inativo'
                print("ID: {0: <3} Nome: {1: <30} Cidade: {2: <25} Situação: {3: <10}".format(str(l[0]),l[1],str(l[2]),posicao ))
                contador+=1
